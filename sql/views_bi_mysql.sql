-- Dolibarr BI Views — MariaDB/MySQL version
...
SELECT d.dt AS date_id, DATE_FORMAT(d.dt, '%Y-%m-%d') AS date_iso,
    YEAR(d.dt) AS annee, QUARTER(d.dt) AS trimestre, MONTH(d.dt) AS mois, DAY(d.dt) AS jour,
    CONCAT(YEAR(d.dt), '-T', QUARTER(d.dt)) AS annee_trimestre,
    DATE_FORMAT(d.dt, '%Y-%m') AS annee_mois,
    CASE MONTH(d.dt) WHEN 1 THEN 'Janvier' WHEN 2 THEN 'Février' WHEN 3 THEN 'Mars' WHEN 4 THEN 'Avril'
    ...
    CASE DAYOFWEEK(d.dt) WHEN 1 THEN 'Dimanche' WHEN 2 THEN 'Lundi' WHEN 3 THEN 'Mardi' WHEN 4 THEN 'Mercredi'
        WHEN 5 THEN 'Jeudi' WHEN 6 THEN 'Vendredi' WHEN 7 THEN 'Samedi' END AS jour_semaine,
    DAYOFWEEK(d.dt) AS num_jour_semaine, WEEK(d.dt) AS semaine_annee,
    CASE WHEN DAYOFWEEK(d.dt) IN (1,7) THEN 'Weekend' ELSE 'Semaine' END AS type_jour,
    CASE WHEN d.dt = CURDATE() THEN 1 ELSE 0 END AS est_aujourdhui,
    CASE WHEN YEAR(d.dt) = YEAR(CURDATE()) THEN 1 ELSE 0 END AS est_annee_courante
FROM (
    SELECT DATE('2020-01-01') + INTERVAL seq DAY AS dt
    FROM (SELECT a.i + b.i*10 + c.i*100 + d.i*1000 AS seq
         ...
         ) seq WHERE DATE('2020-01-01') + INTERVAL seq DAY <= '2030-12-31'
) d;

CREATE OR REPLACE VIEW v_bi_fact_ventes AS
SELECT fd.rowid AS ligne_id, f.rowid AS facture_id, f.ref AS ref_facture,
    f.fk_soc AS client_id, fd.fk_product AS produit_id, DATE(f.datef) AS date_facture,
    DATE(f.date_valid) AS date_validation, DATE(f.date_lim_reglement) AS date_echeance,
...
SELECT ffd.rowid AS ligne_id, ff.rowid AS facture_id, ff.ref AS ref_facture,
    ff.fk_soc AS fournisseur_id, ffd.fk_product AS produit_id, DATE(ff.datef) AS date_facture,
...
SELECT p.rowid AS propal_id, p.ref AS ref_propal, p.fk_soc AS client_id,
    DATE(p.datep) AS date_propal, DATE(p.fin_validite) AS date_expiration,
...
SELECT ps.rowid AS stock_id, ps.fk_product AS produit_id, ps.fk_entrepot AS entrepot_id,
    ...
    COALESCE(IFNULL(NULLIF(p.pmp,0), p.cost_price), 0) * ps.reel AS valeur_stock_cout,
...
SELECT t.rowid AS ticket_id, t.ref AS ref_ticket, t.subject AS sujet, t.type_code AS type_ticket,
    ...
    DATE(t.datec) AS date_creation, DATE(t.date_close) AS date_cloture,
    DATE_FORMAT(t.datec, '%Y-%m') AS mois_creation,
    CASE WHEN t.date_close IS NOT NULL THEN DATEDIFF(t.date_close, t.datec) ELSE NULL END AS delai_resolution_jours,
    t.entity AS entite
